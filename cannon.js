...
class CannonBall {
    ...
    checkArcCollisons(user, line) {
		if(line.intersects(user.arc)){ //Check if ball and arc touch
            ...
            shape.add(this.cannon.position.x,this.cannon.position.y);
            //Add future cannon position
            shape.add(xFuture,yFuture);
            // Current cannon position + previousMove of user
            shape.add(this.cannon.position.x+user.previousMove,this.cannon.position.y);
            ...
            shape.visible = false;
            shape.closed = true;
            //Check if user.arc intersects with this shape
            if(shape.intersects(user.arc)){
                // Offset value depending on if left or right previous user movement
                var offset;
